...
const ServiceCard = ({ title, description, imgSrc }: ServiceCardProps) => {
  ...
  return (
    <div
      ...
      className={`${!isExpanded ? "bg-white" : "bg-[#6591DE] border-[rgba(255,255,255,.4)] border"} rounded-2xl w-full h-full py-3 px-0 md:py-[8%] box-border sm:px-10 shadow-lg md:hover:-translate-y-1 transition-all ease-in-out justify-center items-center flex flex-col relative overflow-hidden sm:pointer-events-none`}
    >
      {/* Card BG Element */}
      ...
      >
        ...
        <h1 className="sm:text-[clamp(10px,5vw,24px)] my-2 leading-tight sm:leading-6.25 md:leading-tight text-center sm:text-left font-bold text-[#6591DE]">
          {title}
        </h1>
      </div>
      ...
      >
        ...
        <h2 className="leading-4 font-bold text-white text-left pt-2">
          {title}
        </h2>
      </div>
      <p
